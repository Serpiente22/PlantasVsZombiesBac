...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleDisconnect(client: Socket) {
    ...
    if (room) this.server.to(room.id).emit('roomUpdated', room);
  }

  @SubscribeMessage('createRoom')
  handleCreateRoom(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string; playerName: string }) {
    const room = this.roomsService.createRoom(data.roomId, { id: client.id, name: data.playerName });
  ...
  handleJoinRoom(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string; playerName: string }) {
    const room = this.roomsService.getRoom(data.roomId);
    const joinedRoom = this.roomsService.joinRoom(data.roomId, { id: client.id, name: data.playerName });
    const isAlreadyIn = room?.players.find((p) => p.id === client.id);

    if (!joinedRoom && !isAlreadyIn) { client.emit('errorJoining', 'Error al unirse.'); return; }
    const activeRoom = joinedRoom || room;
    if (!activeRoom) { client.emit('errorJoining', 'Sala no encontrada'); return; }
    ...
    if (game) {
      ...
      if (p) this.gameService.addPlayerToGame(data.roomId, { id: p.id, name: p.name, color: p.color });
  ...
  handleAddBot(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string }) {
  ...
  handleStartGame(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string }) {
    ...
    room.players.forEach(p => { this.gameService.addPlayerToGame(data.roomId, { id: p.id, name: p.name, color: p.color }); });
    room.status = 'playing';
    this.server.to(data.roomId).emit('roomUpdated', room);
  ...
  handleRollDice(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string }) {
    const game = this.gameService.getGame(data.roomId);
    if (!game || game.status !== 'in-progress') return;
    const current = game.players[game.turnIndex];
    if (current?.id !== client.id || game.dice !== null) return;
    ...
    if (!this.gameService.hasAnyValidMove(data.roomId)) {
      setTimeout(() => { this.advanceAndNotify(data.roomId, current.name, 'no tiene movimientos'); }, 1500);
    } else {
      const state = this.gameService.getPublicGameState(data.roomId);
      if (state) this.server.to(data.roomId).emit('game_state', state);
      this.startTurnTimer(data.roomId); 
  ...
  handleMovePiece(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string; playerId: string; pieceIndex: number }) {
    const game = this.gameService.getGame(data.roomId);
    if (!game || game.status !== 'in-progress') return;
    const current = game.players[game.turnIndex];
    ...
    if (result.eatenPlayerName) this.server.to(data.roomId).emit('killEvent', { killer: current.name, victim: result.eatenPlayerName });
    if (result.powerEffect) this.server.to(data.roomId).emit('powerUpActivated', { player: current.name, effect: result.powerEffect });
  ...
  handleSurrender(@ConnectedSocket() client: Socket, @MessageBody() data: { roomId: string }) {
  ...
  private startTurnTimer(roomId: string) {
      ...
      const timer = setTimeout(() => { this.handleTurnTimeout(roomId); }, 15000); 
  ...
  private executeAutoMove(roomId: string) {
      ...
      if (!this.gameService.hasAnyValidMove(roomId)) {
          this.advanceAndNotify(roomId, current.name, 'tiempo agotado');
      } else {
